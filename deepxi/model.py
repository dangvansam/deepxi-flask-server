...
class DeepXi(DeepXiInput):
	...
	def load_weight(self, model_checkpoint_path, stats_path):
		print('load model checkpoint from:', model_checkpoint_path)
		print('load stats from:', stats_path)
		self.sample_stats(stats_path)
		self.model.load_weights(model_checkpoint_path)
		print('load model weight done')
		#return self.model
		
	def infer_custom(self, x):
		x_spec, x_phase = self.observation(x)
		#print('x_spec:',x_spec.shape)
		x_spec_batch = np.zeros([1, x_spec.shape[0], self.n_feat], np.float32)
		x_spec_batch[0,:,:] = x_spec.numpy()
		#print('x_spec_batch:',x_spec_batch.shape)
		x_predicted = self.model.predict(x_spec_batch)
		#print('x_predicted:',x_predicted.shape)
		#exit()
		xi_hat = self.xi_hat(x_predicted)
		y_STMS = np.multiply(x_spec, gfunc(xi_hat, xi_hat+1, gtype='mmse-lsa'))
		y = self.polar_synthesis(y_STMS, x_phase).numpy()
		#print('predict done')
		# if save_wav_path != None:
		# 	save_wav(save_wav_path, y, self.f_s)
		# 	print('saved wav to:', save_wav_path)
		# else:
		return y
